...
- minSdk 24, targetSdk/compileSdk 37. UI is Compose-only, edge-to-edge, theme in `ui/theme/` with a fixed teal/slate palette (dynamic color is **disabled** by default so the app identity is consistent; the `dynamicColor` flag still exists if ever needed). Spacing is centralized in `ui/theme/Spacing.kt` (`Spacing.xs`→`xxl`, `screenPadding()`, `detailPadding()`); screens must use those helpers rather than hard-coded `dp`. Strings are in `res/values/strings.xml` (UI labels in French); editorial content (titles/costs/steps) is seed data in the asset DB, not string resources.
